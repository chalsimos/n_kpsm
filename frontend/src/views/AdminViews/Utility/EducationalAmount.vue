<template>
        ...
        <div class="p-4 border-2 border-gray-200 border-dashed rounded-lg dark:border-gray-700 mt-14">
            <v-card class="bg-orange-200" flat>
                <v-card-title class="d-flex align-center pe-2 bg-orange-200">
                    <v-icon icon="mdi-hospital-box-outline"></v-icon> &nbsp; Educational Assistance Amount
                    <v-spacer></v-spacer>
                ...
                <v-data-table v-model:search="search" :items="items" :items-per-page="5">
                    ...
                    <template v-slot:item="{ item } " class="bg-orange-200">
                        <tr class="h-[5vh] text-center" elevation="2">
                            ...
                            <td class="whitespace-nowrap uppercase">
                                <v-chip :color="item.status === 'Active' ? 'Red' : 'green'" class="text-uppercase" size="small" label>{{ item.status }}</v-chip>
                            </td>
                            <td class="whitespace-nowrap uppercase">
                                <div class="mb-1">
    ...
    export default {
        ...
        methods: {
            ...
            fetchData() {
                axios.get('/api/educational-assistance/get-all-amount', {
                        headers: {
                            Authorization: `Bearer ${localStorage.getItem('token')}`
                        }
                    })
